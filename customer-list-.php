...
<div class="admin-container">
    ...
    <div class="admin-card">
        ...
        if (!empty($_GET['search'])) {
            $where_conditions[] = "(name LIKE ? OR login LIKE ?)";
        ...
        if (!empty($_GET['point_range'])) {
            switch ($_GET['point_range']) {
                case '0':
                    $where_conditions[] = "point = 0";
                    break;
                case '1-100':
                    $where_conditions[] = "point BETWEEN 1 AND 100";
                    break;
                case '101-500':
                    $where_conditions[] = "point BETWEEN 101 AND 500";
                    break;
                case '501+':
                    $where_conditions[] = "point >= 501";
                    break;
            }
        }
        ...
        $sql = $pdo->prepare("SELECT * FROM customer $where_clause ORDER BY id DESC");
        $sql->execute($params);
                        ...
                        <?php foreach ($customers as $row): ?>
                            <tr class="customer-row" data-customer-id="<?= $row['id'] ?>">
                                    ...
                                    <div class="customer-info">
                                        ...
                                        <div class="customer-meta">
                                            <span class="postal-code">
                                                ...
                                                <?= $row['postcode'] > 0 ? '〒' . str_pad($row['postcode'], 7, '0', STR_PAD_LEFT) : '未設定' ?>
                                            </span>
                                        </div>
                                    </div>
                                </td>
                                <td class="address-cell">
                                    <div class="address-display" title="<?= htmlspecialchars($row['address']) ?>">
                                ...
                                <td class="region-cell">
                                    <?php
                                    $region_name = $regions[$row['region_id']] ?? '未設定';
